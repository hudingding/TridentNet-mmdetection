...
from .tridentresnet import TridentResNet, TridentConv, make_tridentres_layer
...
class TridentBottleneckBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if it is "caffe", the stride-two layer is the first 1x1 conv layer.
        """
        super(TridentBottleneckBlock, self).__init__()
        ...
        self.num_branch = len(self.dilations)
        ...
        self.norm3_name, norm3 = build_norm_layer(
            norm_cfg, self.planes * self.expansion, postfix=3)
        ...
        self.conv2 = TridentConv(
            ...
            kernel_size=3,
            stride=self.conv2_stride,
            paddings=self.dilations,
            bias=False,
            groups=groups,
            dilations=self.dilations,
            num_branch=len(self.dilations),
...
class TridentResNext(TridentResNet):
    """ResNeXt backbone.
    ...
    def __init__(self, groups=64, base_width=4, **kwargs):
        super(TridentResNext, self).__init__(**kwargs)
        self.groups = groups
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            if i < 3:
                ...
                self.block = TridentBottleneckBlock
                res_layer = make_tridentres_layer(
                    self.block,
                    self.inplanes,
                    planes,
                    num_blocks,
